...
**Def.** 对于$M$ 上的除子 $D\geq 0$ , 它所决定的摩天大楼层(skyscraper sheaf) $ç $ 是指在每个开集 $W$ 附着上 $\mathbb{C}^{\oplus\mathrm{deg} D\mid
_
W}.$ 

我们下面写 $\mathrm{dim}H^q(M, \mathcal{F})$ 是指复线性空间 $\mathrm{Hom}(H^q(M, \mathcal{F}), \mathbb{C}$ 的维数.另外对于 $M$ 上一个层 $\mathcal{F}$ 我们形式地定义 $$\chi(\mathcal{F})=\sum
_
