...
date: 2024-02-01
layout: post
title: BNSNote017 Galois Correspondance, Finite Groups, Profinite Groups
...
**Proof.** Let $F$ be a finite field. By the structure theorem of finite abelian groups there exists $x\in F^\times$ of order $d=\mathrm{exp}(F^\times),$ such that for all $y\in F^\times,$ $y^d=1.$ Therefor elements of $F^\times$ are roots of the polynomial $X^d-1,$ which implies $\\# F^\times \leq d$ and hence $F^\times$ is cyclic. $\square$
