...
接下来的note大约还是会回到数论里面去. 可能会写的是轻松愉快的关于用Minkowski理论证明Dirichlet单位定理以及类数有限性这样的话题, 以及 Adèle & Idèle 的基本的语言. 另外之前modular form卡在维数公式之前也是因为需要Riemann-Roch, 所以可以从那里出发继续读modular form了. 还有的是考虑到上述的Riemann-Roch实际上是个函数域的事情, 我们希望它有一个在数域的类似物, 虽然还不知道有什么用但这个是我老百姓近期主要在学的东西 (可见于Neukirch书第三章). 那么我们下次再见.
