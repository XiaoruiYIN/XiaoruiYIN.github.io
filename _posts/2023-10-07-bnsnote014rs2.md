...
依据上面Euler示性数的定义我们容易看到 $$\chi(\Omega (L)) = \chi(\Omega (L-\lambda(D))) + \chi(\mathbf{S}
_
D). $$
...
> 对于足够细的 $M$ 的局部有限开覆盖 $\mathfrak{M}$, 取 $f\in Z^1(\mathfrak(M); \mathbf{S}
_
D))$, 我们对于 $W
_
...
# 3. Hodge定理和 Serre Duality 的证明概要

因为上面最后的Prop3我们在用Hodge定理,所以最后我们也用Hodge来证明Serre Duality. 这部分的形式计算实在是比较大,所以我不敲键盘了,不过会在bns的号上发的时候再附上手抄的笔记. 
