...
> 下面这段我也不知道我在写什么所以全部不打成正文.

> 显然有正合列 $$0\to\Omega^1\to\mathcal{A}^{(1,0)}\xrightarrow[\text{d}]\mathcal{A}^2\to 0.$$
...
i-\omega
_
j$ 是全纯1-form i.e. $\delta\xi\in Z^1(\mathfrac{M};\mathcal{M})$ 其中 $\delta$ 是边缘算子. 对于这样的 $\xi$ 显然可以良定义一点处 $\xi$ 的留数为它每一个组成部分的亚纯函数的留数之和. 我们现在要说明这个定义和我们讲的亚纯微分的留数定义是一样的,换句话说这个留数允许我们把 $H^1(M;\Omega^1)$ 和 $\mathcal{A}^2/\text{d}\mathcal{A}^{(1,0)}$ 混为一谈.

**Lemma.** $\mathrm{Res}(\xi)=\mathrm{Res}([\delta\xi]).$ 其中 $[\delta\xi]$ 是指它在 $H^1(M;\Omega^1)$ 的等价类. 

> 首先我们解释 $\mathrm{Res}([\delta\xi])$. 因为 $\delta\xi\in Z^1(\mathfrac{M};\mathcal{M})\subset \B^1(\mathfrak{M};\mathcal{A}^{(1,0)})$ 所以我们可以设 $(\simga
_
...
事实上我们避开 $D$ 和 $\mathrm{Div}(\omega)$ (注意都是离散的)任意取一点 $P$ 以及附近chart $(W,z),$ 使得 $W$ 当中不含邮前面那些点. 在这个局部我们设 $\omega=f\mathrm{d}z$, 根据假设 $f$ 在这附近不取0, 因此如果我们考虑一个 $\Omega^0(\lambda(D))$ 的0-cochain $\xi$, 它在一个只含2个元素的open cover: $W$ 取 $1/zf$ 而 $M\backslash P$ 取 $0$, 根据上面的引理就应当有 $\mathcal{Res}(\omega\xi)=1.$ $\square$
