...
$$\Phi(x)=\frac{\int\_0^x \psi(t) \mathrm{d}t}{\int\_0^1 \psi(t) \mathrm{d}t},$$
...
$$\mathrm{d}\bigg(\sum\_J f\_J\mathrm{d}x^J\bigg)=\sum\_J \mathrm{d}f\_J\wedge\mathrm{d}x^J.$$
...
$$\int\_\gamma\omega:=\int\_a^b\gamma^\ast\omega=\int\_a^b\omega\_x(\gamma^\prime(x))\mathrm{d}x.$$
...
$$\int\_{F\circ\gamma} \eta=\int\_a^b (F\circ\gamma)^\ast\eta=\int\_a^b \gamma^\ast F^\ast \eta=\int\_\gamma F^\ast \eta.$$
...
$$\int\_a^b\omega=\int\_c^d\varphi^\ast\omega \bigg(\text{resp.} -\int\_c^d\varphi^\ast\omega\bigg).$$
...
$$\int\_\gamma Df:=\int\_a^b\gamma^\ast Df=\int\_a^b Df(\gamma^\prime(x))\mathrm{d}x.$$
...
$$\frac{\partial^2 f}{\partial x\_i\partial x\_j}=\frac{\partial^2 f}{\partial x\_j\partial x\_i},$$
...
$$\partial\_i f(x) = \partial\_i\int\_0^1 f\_i(tx)x\_i\mathrm{d}t \\= \int\_0^1 \bigg(f\_i(tx)+t\frac{\partial f\_i}{\partial x\_i}(tx)\bigg)\mathrm{d}t \\= \int\_0^1\frac{\mathrm{d}}{\mathrm{d}t}\bigg(tf\_i(tx)\bigg)\mathrm{d}t=f\_i(x).$$
...
这个当然和这门课无关, 但是既然都写到这里了就顺便试着写一点. 细节可看 Bott&Tu 的 GTM94.

通过外导数我们得到上链复形

$$0\to\Omega^0(M)\to\Omega^1(M)\to\Omega^2(M)\to...$$

它的上同调群(是实线性空间)称为 de Rham 上同调. 更具体地说我们称 $\mathrm{d}:\Omega^k(M)\to\Omega^{k+1}(M)$ 的核 $Z^k(M)$ 是闭 $k$ -形式, 像 $B^{k+1}(M)$ 是恰当 $k+1$ -形式, de Rham 上同调群 $$H^k\_{\mathrm{dR}}(M)=Z^k(M)/B^k(M).$$

特别地, 上面证明的 Poincaré 引理就是说对于 $U\subset\mathbb{R}^n$ 星形 有 $H^1\_{\mathrm{dR}}(U)=0.$
...
$$\int\_D\omega=\int\_D f\mathrm{d}x\_1 ...\mathrm{d}x\_n.$$


现在设 $G:D\to E$ 是两个可以定义积分的开集之间的保向的光滑同胚, 并且连续地延拓到闭包上. 对于 $E$ 上的 $n$-形式 $\omega$ 根据重积分的换元我们有

$$\int\_E \omega=\int\_E f\mathrm{d}V=\int\_D (f\circ G)(\mathrm{det}DG)\mathrm{d}V=\int\_D G^\ast\omega.$$
...
$$\int\_M\omega=\int\_D (\varphi^{-1})^\ast \omega.$$
...
$$\int\_M\omega=\sum\_{\alpha\in\Lambda}\psi\_\alpha \omega.$$
...
$$\int\_M\omega=\sum\_{i=1}^r\int\_{D\_i}F\_i^\ast\omega.$$
...
$$\int\_M \mathrm{d}\omega=\int\_{\partial M}\omega.$$
