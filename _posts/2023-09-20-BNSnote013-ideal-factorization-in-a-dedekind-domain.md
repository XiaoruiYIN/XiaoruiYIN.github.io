...
**Theorème(Dedekind)**. Supposons que $p$ est un entier premier ne divisant pas l'indice of $\mathbb{Z}[\theta]$ dans $\mathcal{O}_K$, 
  $\theta$ est integral sur $\mathbb{Q}$ ayant le polynôme minimal $F_{\theta}(X)$, son image canonique dans $\mathbb{Z}/p\mathbb{Z}$ est factorisé dans un produit de polynômes irréductibles $\widetilde{F}=\prod_{k=1}^g\widetilde{F_k}^{e_k}$.
